...
class RunCode implements OnClickListener, Runnable {
	...
	private static final String[] msg = { "等待", "等待重判", "编译中", "运行并评判", "正确", "格式错误", "答案错误(点击看对比)", "时间超限", "内存超限", "输出超限", "运行错误", "编译错误", "编译成功", "运行完成" };
	...
	public void onClick(View v) {
		...
		String string = source.getText().toString();
		...
		params.add(new BasicNameValuePair("input_text", console.getText().toString()));
	...
	private String httpPost(List<NameValuePair> params) {
		...
		try {
			...
			if (httpResponse.getStatusLine().getStatusCode() == 200) {
				...
				ret = ("Error Response" + httpResponse.getStatusLine().toString());
			}
	...
	public void run() {
		try {
			...
			String resultURL = "http://hustoj.sinaapp.com/status-ajax.php?solution_id=" + sid;
			int result = 0;
			last = -1;
			do {
				...
				if (ret.indexOf(",") != -1) {
					...
					if (result > last) {
						...
						handler.post(new Runnable() {
							...
							public void run() {
								try {
									Toast.makeText(RunCode.this.activity, RunCode.this.msg[RunCode.this.last], Toast.LENGTH_LONG).show();
								} catch (Exception e) {
									e.printStackTrace();
								}
							}
			...
			String outURL = "http://hustoj.sinaapp.com/status-ajax.php?tr=&solution_id=" + sid;
			ret = HttpGet(outURL);
