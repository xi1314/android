 ...
 *         <p/>
 *         Collected from https://github.com/yahoo/android-range-seek-bar
 */
...
//    private final float thumbHalfHeight = 0.5f * thumbImage.getHeight();
    ...
    private RectF mRect;
    String label_min, label_max;
    int res_thumb_normal, res_thumb_pressed, res_thumb_disabled, res_track_active, res_track_inactive;
    int thumb_size = 24;
    int track_height = 2;
//    int res_color_inactive, res_color_active;

    private static final int DEFAULT_TEXT_SIZE_IN_DP = 14;
    private static final int DEFAULT_TEXT_DISTANCE_TO_BUTTON_IN_DP = 8;
    private static final int DEFAULT_TEXT_DISTANCE_TO_TOP_IN_DP = 8;
    private boolean mSingleThumb;
    private Drawable d_track_active, d_track_inactive;
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        thumb_size = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, thumb_size, dm);
        if (attrs == null) {
            ...
            label_max = a.getString(R.styleable.RangeSeekBar_maxLabel);
            if (label_max == null) {
                label_max = "";
            }
            label_min = a.getString(R.styleable.RangeSeekBar_minLabel);
            if (label_min == null) {
                label_min = "";
            }
            res_thumb_normal = a.getResourceId(R.styleable.RangeSeekBar_thumb_normal, R.drawable.layer_seekbar_normal);
            res_thumb_pressed = a.getResourceId(R.styleable.RangeSeekBar_thumb_pressed, R.drawable.layer_seekbar_pressed);
            res_thumb_disabled = a.getResourceId(R.styleable.RangeSeekBar_thumb_disabled, R.drawable.layer_seekbar_disabled);
            thumb_size = a.getDimensionPixelSize(R.styleable.RangeSeekBar_thumb_size, thumb_size);
            track_height = a.getDimensionPixelSize(R.styleable.RangeSeekBar_track_height, track_height);
            res_track_active = a.getResourceId(R.styleable.RangeSeekBar_track_active, R.drawable.layer_seekbar_track_active);
            res_track_inactive = a.getResourceId(R.styleable.RangeSeekBar_track_inactive, R.drawable.layer_seekbar_track_inactive);
//            res_track = a.getResourceId(R.styleable.RangeSeekBar_track, R.drawable.layer_seekbar_disabled);
            a.recycle();
        }
        d_track_active = getResources().getDrawable(res_track_active);
        d_track_inactive = getResources().getDrawable(res_track_inactive);
        Drawable layer = getResources().getDrawable(res_thumb_normal);
//        GradientDrawable d = (GradientDrawable) layer.findDrawableByLayerId(R.id.seekbar_thumb_item);
        int width = thumb_size;
        int height = thumb_size;
        layer.setBounds(0, 0, width, height);
        thumbImage = Bitmap.createBitmap(width, height, Bitmap.Config.ARGB_8888);
        layer.draw(new Canvas(thumbImage));
        layer = getResources().getDrawable(res_thumb_pressed);
//        d = (GradientDrawable) layer.findDrawableByLayerId(R.id.seekbar_thumb_item);
//        width = d.getIntrinsicWidth();
//        height = d.getIntrinsicHeight();
        layer.setBounds(0, 0, width, height);
        thumbPressedImage = Bitmap.createBitmap(width, height, Bitmap.Config.ARGB_8888);
        layer.draw(new Canvas(thumbPressedImage));
        layer = getResources().getDrawable(res_thumb_disabled);
//        d = (GradientDrawable) layer.findDrawableByLayerId(R.id.seekbar_thumb_item);
//        width = d.getIntrinsicWidth();
//        height = d.getIntrinsicHeight();
        layer.setBounds(0, 0, width, height);
        thumbDisabledImage = Bitmap.createBitmap(width, height, Bitmap.Config.ARGB_8888);
        layer.draw(new Canvas(thumbDisabledImage));
        ...
        INITIAL_PADDING = Util.getPX(INITIAL_PADDING_IN_DP);

        mTextSize = Util.getPX(DEFAULT_TEXT_SIZE_IN_DP);
        mDistanceToTop = Util.getPX(DEFAULT_TEXT_DISTANCE_TO_TOP_IN_DP);
        mTextOffset = this.mTextSize + Util.getPX(
                DEFAULT_TEXT_DISTANCE_TO_BUTTON_IN_DP) + this.mDistanceToTop;

//        float lineHeight = Util.getPX(LINE_HEIGHT_IN_DP);
        mRect = new RectF(padding,
                mTextOffset + thumbHalfHeight - track_height / 2,
                getWidth() - padding,
                mTextOffset + thumbHalfHeight + track_height / 2);
//        mRect = new RectF(padding,
...
//                mTextOffset + thumbHalfHeight + lineHeight / 2);
    ...
    protected synchronized void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int height = thumbImage.getHeight() + Util.getPX(HEIGHT_IN_DP);
    ...
    protected synchronized void onDraw(Canvas canvas) {
        ...
        String minLabel = label_min;
        String maxLabel = label_max;
        ...
        d_track_inactive.setBounds((int) mRect.left, (int) mRect.top, (int) mRect.right, (int) mRect.bottom);
//        canvas.drawRect(mRect, paint);
        d_track_inactive.draw(canvas);
        boolean selectedValuesAreDefault = (getSelectedMinValue().equals(getAbsoluteMinValue()) &&
                getSelectedMaxValue().equals(getAbsoluteMaxValue()));
//        float rec1=mRect.left;
//        float rec2=mRect.right;

//        int colorToUseForButtonsAndHighlightedLine = selectedValuesAreDefault ?
//                res_color_inactive :    // default values
//                res_color_active; //non default, filter is active

        // draw seek bar active range line
        mRect.left = normalizedToScreen(normalizedMinValue);
        mRect.right = normalizedToScreen(normalizedMaxValue);
        d_track_active.setBounds((int) mRect.left, (int) mRect.top, (int) mRect.right, (int) mRect.bottom);
//        canvas.drawRect(mRect, paint);
        d_track_active.draw(canvas);
//        float rec3=mRect.left;
//        float rec4=mRect.right;
//        Logger.out("rect:["+rec1+","+rec3+","+rec4+","+rec2+"]");
...
//        canvas.drawRect(mRect, paint);
        ...
        if (!selectedValuesAreDefault) {
            ...
            int offset = Util.getPX(TEXT_LATERAL_PADDING_IN_DP);
            ...
            if (!mSingleThumb) {
                float x = normalizedToScreen(normalizedMinValue) - minTextWidth * 0.5f;
                canvas.drawText(minText, x, mDistanceToTop + mTextSize, paint);
