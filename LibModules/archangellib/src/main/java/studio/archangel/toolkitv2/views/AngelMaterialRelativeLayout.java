...
public class AngelMaterialRelativeLayout extends RelativeLayout {
    ...
    private AngelMaterialProperties.TriggerMode trigger_mode;//触发模式
    ...
    private long duration_total = 200;//动画时间，修改这个可以改变点击相应时间
    ...
    public AngelMaterialRelativeLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public AngelMaterialRelativeLayout(Context context, AttributeSet attrs, int defStyleAttr) {
...
//        Logger.out("("+Color.alpha(color)+","+Color.red(color)+","+Color.green(color)+","+Color.blue(color)+")");
    ...
    WeakHandler handler_click = new WeakHandler(new Handler.Callback() {
        ...
        public boolean handleMessage(Message msg) {
            System.gc();
            if (listener != null)
                listener.onClick(AngelMaterialRelativeLayout.this);
            return false;
        }
    });
...
//            }
    ...
    protected void generateColors(int color) {
        if (paint == null) {
            return;
        }
        color_normal = color;
