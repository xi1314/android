...
public class Kanner extends FrameLayout {
    ...
    private void initLayout() {
        ...
        View view = LayoutInflater.from(context)
                                  .inflate(R.layout.kanner_layout, this, true);
    ...
    private void initImgFromRes(int[] imagesRes) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            iv_dot.setImageResource(R.drawable.dot_white);
            ll_dot.addView(iv_dot, params);
            iv_dots.add(iv_dot);
        }
        iv_dots.get(0).setImageResource(R.drawable.circle_hx_main);

        for (int i = 0; i <= count + 1; i++) {
            ImageView iv = new ImageView(context);
            iv.setScaleType(ScaleType.FIT_XY);
            iv.setBackgroundResource(R.drawable.ic_logo);
            if (i == 0) {
    ...
    private void initImgFromNet(List<String> imagesUrl) {
        ...
        if (count > 0) {
            int w = UiUtil.getDisplayWidth(getContext());
            int h = UiUtil.dip2px(getContext(), 104);
            for (int i = 0; i <= count + 1; i++) {
                ...
                if (i == 0) {
                    HXImageLoader.loadImage(iv, imagesUrl.get(count - 1), w, h);
                    iv.setOnClickListener(new ItemClickListener(count - 1));
                } else if (i == count + 1) {
                    HXImageLoader.loadImage(iv, imagesUrl.get(0), w, h);
                    iv.setOnClickListener(new ItemClickListener(0));
                } else {
                    HXImageLoader.loadImage(iv, imagesUrl.get(i - 1), w, h);
                    iv.setOnClickListener(new ItemClickListener(i - 1));
                }
                imageViews.add(iv);
            }
        }
    ...
    class MyOnPageChangeListener implements OnPageChangeListener {
        ...
        public void onPageSelected(int arg0) {
            for (int i = 0; i < iv_dots.size(); i++) {
                if (i == arg0 - 1) {
                    iv_dots.get(i).setImageResource(R.drawable.circle_hx_main);
                } else {
                    iv_dots.get(i).setImageResource(R.drawable.dot_white);
                }
            }
        }
    }
