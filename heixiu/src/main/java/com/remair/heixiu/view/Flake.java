...
public class Flake {
    ...
    static Flake createFlake(float xRange, Bitmap originalBitmap) {
        ...
        flake.width = (int) (100 + (float) Math.random() * 50);
        ...
        flake.speed = 200 + (float) Math.random() * 1000;
        //        flake.speed = 220;
        ...
        if (flake.bitmap == null) {
            flake.bitmap = Bitmap
                    .createScaledBitmap(originalBitmap, flake.width, flake.height, true);
    ...
    static Flake createFlakeBig(float xRange, Bitmap originalBitmap, boolean big) {
        Flake flake = new Flake();
        // Size each flake with a width between 5 and 55 and a proportional height
        if (big) {
            flake.width = (int) (60 + (float) Math.random() * 20);
