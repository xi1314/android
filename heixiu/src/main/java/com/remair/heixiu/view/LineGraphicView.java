        ...
        for (int i = 0; i < spacingHeight + 1; i++) {
            canvas.drawLine(blwidh,
                    bheight - (bheight / spacingHeight) * i + marginTop, (
                            canvasWidth - blwidh),
                    bheight - (bheight / spacingHeight) * i +
                            marginTop, mPaint);// Y坐标

            if (i == spacingHeight || i == 0) {
                //	String.valueOf(averageValue * ( spacingHeight )).length()*dip2px(12)
                drawText(String.valueOf(averageValue * i),
                        blwidh / 2, bheight - (bheight / spacingHeight) * i +
                                marginTop, canvas);
            }
        }
    }
    ...
    private void drawAllYLine(Canvas canvas) {
        for (int i = 0; i < yRawData.size(); i++) {
            ...
            if (i == 0) {
                ...
                canvas.drawLine(blwidh +
                                (canvasWidth - blwidh) / yRawData.size() * i, marginTop,
