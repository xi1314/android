...
public class PanningViewAttacher implements ViewTreeObserver.OnGlobalLayoutListener {
    ...
    public static final int DEFAULT_PANNING_DURATION_IN_MS = 50000;
    ...
    public PanningViewAttacher(ImageView imageView, long duration) {
        ...
        mIsPortrait = imageView.getResources().getConfiguration().orientation ==
        ...
        if (mIsPortrait) {
            if (mWay == Way.R2L) {
                animate(mDisplayRect.left, mDisplayRect.left -
                        (mDisplayRect.right -
                                 getImageViewWidth()), remainingDuration);
            } else {
            ...
            if (mWay == Way.B2T) {
                animate(mDisplayRect.top, mDisplayRect.top -
                        (mDisplayRect.bottom -
                                 getImageViewHeight()), remainingDuration);
            } else {
    ...
    private void animate(float start, float end, long duration) {
        Log.d(TAG,
                "startPanning : " + start + " to " + end + ", in " + duration +
                        "ms");
    ...
    private void refreshDisplayRect() {
        mDisplayRect
                .set(0, 0, getDrawableIntrinsicWidth(), getDrawableIntrinsicHeight());
