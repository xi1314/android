 ...
 */
public class PriLetterAdapterA<T> extends CommonRecyclerAdapter {

    private int mTag;
    private FragmentManager childFragmentManager;

    public PriLetterAdapterA(Context context, List<T> mList, FragmentManager childFragmentManager, int tag) {
    ...
    public class ItemViewHolder extends AngelCommonViewHolder {
        ...
        @BindView(R.id.chat_msg_count)
        TextView chat_msg_count;
        @BindView(R.id.chat_name)
        TextView chat_name;
        @BindView(R.id.chat_sex)
        ImageView chat_sex;
        @BindView(R.id.chat_levl)
        SimpleDraweeView chat_levl;
        @BindView(R.id.chat_grad)
        TextView chat_grad;
        ...
        @BindView(R.id.chat_private)
        TextView chat_private;
        ...
        public void render(final Context context, Object model) {
            ConcernInfoDB concerinfo = null;
            UnFollowConcernInfoDB unconcerinfo = null;
            if (mTag == 0) {
                concerinfo = (ConcernInfoDB) model;
                setData(concerinfo, context);
            } else if (mTag == 1) {
                unconcerinfo = (UnFollowConcernInfoDB) model;
                setData(unconcerinfo, context);
            }
        }

        private void setData(ConcernInfoDB concerinfo, Context context) {
            ...
            try {
                ...
                if ("-1".equals(concerinfo.getHxtype())) {
                    ...
                    HXImageLoader.loadImage(chat_img, poto, Utils.getPX(42), Utils.getPX(42));
                    chat_name.setText(name);
                    chat_private.setBackgroundResource(R.drawable.shape_chat_type);
                    chat_private.setOnClickListener(new View.OnClickListener() {
                        ...
                        public void onClick(View view) {
                            ...
                            chatMessageDialog.setStyle(DialogFragment.STYLE_NO_TITLE, R.style.add_dialog);
                            chatMessageDialog.show(childFragmentManager, "private");
                        }
                    });
                } else {
                    chat_private.setVisibility(View.VISIBLE);
                    ...
                    chat_private.setText(HXUtil.GetStringFormat(concerinfo.getUpdatetime()));
                    if ("1".equals(user_id)) {
                        chat_name.setText(name);
                        final SpannableString spanString = StringUtil.stringToSpannableString(concerinfo.getLastmessage(), context);
                        ...
                        if (SharedPreferenceUtil.getLong(user_id, 0) > 0) {
                            ...
                            chat_msg_count.setText(SharedPreferenceUtil.getLong(user_id, 0) + "");
                        } else {
                            chat_msg_tip.setVisibility(View.GONE);
                        }
                    } else {
        ...
        private void setData(UnFollowConcernInfoDB unconcerinfo, Context context) {
            ...
            try {
                ...
                if ("-1".equals(unconcerinfo.getHxtype())) {
                    ...
                    rl_grade.setVisibility(View.VISIBLE);
                    chat_msg.setVisibility(View.VISIBLE);
                    chat_private.setText(HXUtil.GetStringFormat(unconcerinfo.getUpdatetime()));
                    if (gender == 0) {
                        chat_sex.setImageResource(R.drawable.sex_man);
                    } else {
                        chat_sex.setImageResource(R.drawable.sex_woman);
                    ...
                    Xtgrade.mXtgrade(grade, chat_levl, chat_grad);
                    chat_name.setText(name);
                    final SpannableString spanString = StringUtil.stringToSpannableString(unconcerinfo.getLastmessage(), context);
                    chat_msg.setText(spanString);
                    if (SharedPreferenceUtil.getLong(user_id, 0) > 0) {
                        chat_msg_tip.setVisibility(View.VISIBLE);
                        chat_msg_count.setText(SharedPreferenceUtil.getLong(user_id, 0) + "");
