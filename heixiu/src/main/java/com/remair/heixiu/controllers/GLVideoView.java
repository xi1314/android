...
public class GLVideoView extends GLView {
        ...
        mYuvTexture.setGLRenderListener(new YUVTexture.GLRenderListener() {
            ...
            public void onRenderInfoNotify(int width, int height, int angle) {
                if (QLog.isColorLevel()) {
                    Logger.out("onRenderInfoNotify uin: " + mIdentifier + ", mVideoSrcType: " + mVideoSrcType + ", width: " + width + ", height: " + height + ", angle: " + angle);
                }
                mYuvTexture.setTextureSize(width, height);
                // refresh();
                invalidate();
                mContext.sendBroadcast(new Intent(Utils.ACTION_HAVE_REMOTEVIDEO));
            }
        });
    ...
    protected void renderBackground(GLCanvas canvas) {
        ...
        if (mGlVersion == -1) {
            mGlVersion = com.tencent.av.opengl.utils.Utils.getGLVersion(mContext);
        }
    ...
    protected void render(GLCanvas canvas) {
        ...
        if (null != mIdentifier && mYuvTexture != null && mYuvTexture.canRender() && mNeedRenderVideo) {
            ...
            if (!mDragging && mPosition != NONE) {
                ...
                mPosition = NONE;
                if (QLog.isColorLevel()) {
                    Logger.out("render uin: " + mIdentifier + ", mVideoSrcType: " + mVideoSrcType + ", width: " + width + ", height: " + height + ", mScale: " + mScale + ", mPivotX: " + mPivotX + ", mPivotY: " + mPivotY + ", x: " + x + ", y: " + y + ", w: " + w + ", h: " + h + ", mOffsetX: " + mOffsetX + ", mOffsetY: " + mOffsetY + ", mWidth: " + mWidth + ", mHeight: " + mHeight);
                }
            }
            x += mOffsetX;
            y += mOffsetY;
            ...
            if (sRatio > tRatio) {
                ...
                if (com.tencent.av.opengl.utils.Utils.getGLVersion(mContext) == 1) {
                    newSourceW = imgW * newSourceW / com.tencent.av.opengl.utils.Utils
                            .nextPowerOf2((int) imgW);
                }
                ...
                if (com.tencent.av.opengl.utils.Utils.getGLVersion(mContext) == 1) {
                    newSourceH = imgH * newSourceH / com.tencent.av.opengl.utils.Utils
                            .nextPowerOf2((int) imgH);
                }
        ...
        if (mStringTexture != null) {
            ...
            Log.e("TTTT", "dfdskafjdklajfdjald");
    ...
    public void setRender(String identifier, int videoSrcType) {
        Logger.out("setRender identifier" + identifier + " videoSrcType" + videoSrcType);
        Log.e("TTT", "setRender identifier" + identifier + " videoSrcType" + videoSrcType);
        if (null == identifier) {
            mIdentifier = null;
            mVideoSrcType = 0;
            return;
    ...
    public void setIsPC(boolean isPC) {
        if (QLog.isColorLevel()) {
            Logger.out("setBlackBorder uin: " + mIdentifier + ", mVideoSrcType: " + mVideoSrcType + ", mIsPC: " + mIsPC + ", isPC: " + isPC);
        }
        if (mIsPC != isPC) {
            mIsPC = isPC;
            mScale = 1.0f;
    ...
    public void setMirror(boolean mirror) {
        if (QLog.isColorLevel()) {
            Logger.out("setMirror uin: " + mIdentifier + ", mVideoSrcType: " + mVideoSrcType + ", mMirror: " + mMirror + ", mirror: " + mirror);
        }
        if (mMirror != mirror) {
            mMirror = mirror;
            invalidate();
    ...
    public void setScale(float scale, int x, int y, boolean isEnd) {
        if (QLog.isColorLevel()) {
            Logger.out("setScale uin: " + mIdentifier + ", mVideoSrcType: " + mVideoSrcType + ", scale: " + scale + ", x: " + x + ", y: " + y + ", isEnd: " + isEnd + ", mOffsetX: " + mOffsetX + ", mOffsetY: " + mOffsetY + ", mX: " + mX + ", mY: " + mY + ", mWidth: " + mWidth + ", mHeight: " + mHeight);
        }
        // mZooming = !isEnd;
        if (isEnd) {
            if (scale < 1.0f) {
    ...
    public void setOffset(int offsetX, int offsetY, boolean isEnd) {
        if (QLog.isColorLevel()) {
            Logger.out("setOffset uin: " + mIdentifier + ", mVideoSrcType: " + mVideoSrcType + ", offsetX: " + offsetX + ", offsetY: " + offsetY + ", isEnd: " + isEnd);
        }
        mDragging = !isEnd;
        if (isEnd) {
            Rect p = getPaddings();
            ...
            if (QLog.isColorLevel()) {
                Logger.out("setOffset uin: " + mIdentifier + ", mVideoSrcType: " + mVideoSrcType + ", mPosition: " + mPosition + ", width: " + width + ", height: " + height + ", mX: " + mX + ", mY: " + mY + ", mWidth: " + mWidth + ", mHeight: " + mHeight);
            }
        }
        int x = offsetX;
        int y = offsetY;
    ...
    public void setText(String text, float textSize, int color) {
        if (QLog.isColorLevel()) {
            Logger.out("setText uin: " + mIdentifier + ", mVideoSrcType: " + mVideoSrcType + ", text: " + text + ", textSize: " + textSize + ", color: " + color);
        }
        if (mStringTexture != null) {
            mStringTexture.recycle();
            mStringTexture = null;
    ...
    public void enableLoading(boolean loading) {
        if (mLoading != loading) {
            if (QLog.isColorLevel()) {
                Logger.out("enableLoading uin: " + mIdentifier + ", mVideoSrcType: " + mVideoSrcType + ", loading: " + loading + ", mLoading: " + mLoading);
            }
            mLoading = loading;
            if (loading) {
                if (mLoadingTexture == null) {
    ...
    public void setNeedRenderVideo(boolean needRender) {
        if (QLog.isColorLevel()) {
            Logger.out("setNeedRenderVideo uin: " + mIdentifier + ", mVideoSrcType: " + mVideoSrcType + ", bRender: " + needRender + ", mNeedRenderVideo: " + mNeedRenderVideo);
